...
function setUserNameAndPassWord() {
    ...
    console.log(gradient('orange', 'yellow').multiline(["█▀█ █▀█ █▀█ ░░█ █▀▀ ░█░█▄▒▄█▒█▀▄▒▄▀▄░█▄░█", "█▀▀ █▀▄ █▄█ █▄█ ██▄ ░█░█▒▀▒█░█▀▄░█▀█░█▒▀█",].join('\n')));
    console.log();
    console.log(gradient("orange","yellow",)('[ FCA-PROJECT-IMRAN ] > Operating System: ') + (os.type()));
    console.log(gradient("orange","yellow",)('[ FCA-PROJECT-IMRAN ] > Machine Version: ') + (os.version()));
    console.log(gradient("orange","yellow",)('[ FCA-PROJECT-IMRAN ] > FCA Version: ') + (global.Fca.Version) + '\n');
        ...
        rl.question(Language.TypeAccount, (Account) => {
            if (!Account.includes("@") && global.Fca.Require.utils.getType(parseInt(Account)) != "Number") return logger.Normal(Language.TypeAccountError, function () { process.exit(1) }); //Very Human
                    ...
                    if (global.Fca.Require.FastConfig.ResetDataLogin) {
                        ...
                        global.Fca.Require.fs.writeFileSync(process.cwd() + '/ImranFca.json', JSON.stringify(global.Fca.Require.FastConfig, null, 4));
                    }
        ...
        if (global.Fca.Require.FastConfig.Websocket_Extension.Status) {
            ...
            if (!UserName || !PassWord || global.Fca.Require.FastConfig.Websocket_Extension.ResetData) {
                ...
                if (!UserName || !PassWord) {
                    ...
                    global.Fca.Require.fs.writeFileSync(process.cwd() + "/ImranFca.json", JSON.stringify(global.Fca.Require.FastConfig, null, "\t"));
                }
                else {
                    try {
