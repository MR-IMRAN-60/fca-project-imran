...
try {
    ...
    if (!global.Fca.Require.fs.existsSync(process.cwd() + '/ImranFca.json')) {
        global.Fca.Require.fs.writeFileSync(process.cwd() + "/ImranFca.json", JSON.stringify(global.Fca.Data.ObjFastConfig, null, "\t"));
        process.exit(1);
    }

try {
    var Data_Setting = require(process.cwd() + "/ImranFca.json");
}
catch (e) {
    global.Fca.Require.logger.Error('Detect Your /ImranFca.json Settings Invalid!, Carry out default restoration');
    global.Fca.Require.fs.writeFileSync(process.cwd() + "/ImranFca.json", JSON.stringify(global.Fca.Data.ObjFastConfig, null, "\t"));     
    process.exit(1)
}
    if (global.Fca.Require.fs.existsSync(process.cwd() + '/ImranFca.json')) {
        ...
        for (let i of All_Variable) {
            if (Data_Setting[i] == undefined) {
                ...
                global.Fca.Require.fs.writeFileSync(process.cwd() + "/ImranFca.json", JSON.stringify(Data_Setting, null, "\t"));
            }
            else continue; 
        } //Check Variable
...
module.exports = function(loginData, options, callback) {
    ...
    return got.get('https://raw.githubusercontent.com/ivancotacte/Global_fca-project-orion/main/InstantAction.json').then(async function(res) {
        ...
        console.log(err)
            ...
            log.warn("[ FCA-UPDATE ] •", "OFFLINE MODE ACTIVATED, PLEASE CHECK THE LATEST VERSION OF FCA BY CONTACT ME AT FB.COM/Imran.Ahmed");
        return login(loginData, options, callback);
    });
};